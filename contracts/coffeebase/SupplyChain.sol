...
contract SupplyChain is ConsumerRole, DistributorRole, RetailerRole, FarmerRole {
  ...
  modifier paidEnough(uint _price) { 
    require(msg.value >= _price, 'Not enough ether inserted!'); 
    _;
  }
  ...
  modifier sold(uint _upc) {
    require(items[_upc].itemState == State.Sold, "Item hasn't been sold!");
    _;
  }
  
  // Define a modifier that checks if an item.state of a upc is Shipped
  modifier shipped(uint _upc) {
    require(items[_upc].itemState == State.Shipped, "Item hasn't been shipped!");
    _;
  }

  // Define a modifier that checks if an item.state of a upc is Received
  modifier received(uint _upc) {
    require(items[_upc].itemState == State.Received, "Item hasn't been recieved!");
    _;
  }

  // Define a modifier that checks if an item.state of a upc is Purchased
  modifier purchased(uint _upc) {
    require(items[_upc].itemState == State.Purchased, "Item hasn't been purchased!");
    _;
  }
  ...
  (uint itemSKU, uint itemUPC, address ownerID, address originFarmerID, string originFarmName, string originFarmInformation, string originFarmLatitude, string originFarmLongitude) {
  ...
  (uint itemSKU, uint itemUPC, uint productID, string productNotes, uint productPrice, uint itemState, address distributorID, address retailerID, address consumerID) {
